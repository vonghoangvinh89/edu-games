...
int main() {
	...
	int totalScore = 0; //for keeping total score for the session
	...
	while (playerContinue == 'y') {
		...
		while (victory == false) {
			...
			//gives advice to player depending on where their incorrect guess was
				...
				cout << "\nNot quite! ";
				if (targetWord.compare(guessWord) > 0) {
					cout << "I'll give you a hint: you're looking for a word further down on the list.\n\n";
				}
				if (targetWord.compare(guessWord) < 0) {
					cout << "I'll give you a hint: you're looking for a word higher up on the list.\n\n";
				}
			}
		}
	...
	cout << "Your total score was " << totalScore << " points!\n";
	return 0;
}
